...
@Component({
  ...
  styleUrl: './filtro-pelicula.component.css',
})
export class FiltroPeliculaComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.generosService.obtenerTodos().subscribe((generos) => {
      ...
      .subscribe((valores) => {
        ...
        this.ecribirParametrosBusquedaUrl(valores as filtroPelicula);
      });
    });
  }
  ...
  buscarPeliculas(valores: filtroPelicula) {
    ...
    this.peliculasService.filtrar(valores).subscribe((respuesta) => {
      ...
      const cabecera = respuesta.headers.get('cantidadTotalRegistro') as string;
      this.cantidadTotalRegistros = parseInt(cabecera, 10);
    });
  }

  ecribirParametrosBusquedaUrl(valores: filtroPelicula) {
    let queryStrings = [];
