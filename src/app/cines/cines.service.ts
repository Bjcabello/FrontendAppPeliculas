...
import { IServiciosCRUD } from '../compartidos/interfaces/IServiciosCRUD';
...
export class CinesService implements IServiciosCRUD<CineDTO, CineCreacionDTO> {
  private http = inject(HttpClient);
  private urlBase = environment.apiUrl + '/cines';


  obtenerPaginado(paginacion: PaginacionDTO): Observable<HttpResponse<CineDTO[]>> {
    const queryParams = construirQueryParams(paginacion);
    return this.http.get<CineDTO[]>(this.urlBase, {params: queryParams,
      observe: 'response',
    });
  }
